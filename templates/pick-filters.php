<fieldset class="pad10" style="width: 1000px">
	<form action="#" method="post" id="pick-filters-form" onsubmit="saveFilters(); return false">
		...
		<?php
		$forms = $this->model->_AdminFront->getFiltersForms(true);
		$currentFilters = [];
		...
		$customFilters = $this->model->_Admin->customFiltersForm->getDataset();
		...
		foreach ($campi as $k => $col) {
			...
			<div class="grid">
				...
				<div class="w75" style="vertical-align: middle">
					...
					if ($k !== 'all') {
						$numeric = false;
						if ((!isset($form[$k]) or !in_array($form[$k]->options['type'], ['checkbox', 'radio', 'select'])) and $col !== true and !$col['foreign_key']) {
							switch ($col['type']) {
								case 'tinyint':
								case 'smallint':
								case 'int':
						...
						if (!isset($form[$k]) or !in_array($form[$k]->options['type'], ['checkbox'])) {
							?>
							<input type="radio" name="<?= $k ?>-type" data-managefilters="<?= $k ?>-type" id="campo-<?= $k ?>-type-!=" value="!="<?= (isset($currentFilters[$k]) and $currentFilters[$k]['type'] == '!=') ? ' checked' : '' ?> />
							<label for="campo-<?= $k ?>-type-!=">!=</label>
