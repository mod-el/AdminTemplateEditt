<fieldset class="pad10">
    <form action="?" method="post" id="pick-search-fields-form" onsubmit="saveSearchFields(); return false">
		...
		$setColumns = isset($_COOKIE['model-admin-' . $this->model->_AdminFront->request[0] . '-searchFields']) ? json_decode($_COOKIE['model-admin-' . $this->model->_AdminFront->request[0] . '-searchFields'], true) : array();
		...
		if ($this->model->isLoaded('Multilang') and array_key_exists($this->model->_Admin->options['table'], $this->model->_Multilang->tables)) {
			...
			foreach ($mlTableModel->columns as $k => $col) {
				if ($k === $mlTableModel->primary or isset($columns[$k]) or $k === $mlTableOptions['keyfield'] or $k === $mlTableOptions['lang'])
					continue;
				$columns[$k] = $col;
			}
		}
		...
		foreach ($columns as $k => $col) {
			if ($tableModel->primary === $k or $col['foreign_key'] or !in_array($col['type'], [
					'tinyint',
					'smallint',
					'int',
					'mediumint',
