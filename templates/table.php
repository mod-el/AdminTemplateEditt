...
<div class="pad20o no-overflow">
    <div id="results-table-count">
        ...
        <span class="nowrap">[<a href="?nopag=1"
                                 onclick="allInOnePage(); return false"> tutti su una pagina </a>]</span>
    </div>
    ...
    <div>
		...
		foreach ($this->options['data']['columns'] as $column_id => $f) {
            ...
            <div style="width: <?= $this->model->_ResizeTable->widths[$column_id] ?>px" data-column="<?= $column_id ?>"
                 id="column-<?= $column_id ?>">
                <div class="table-headings-resize"
                     onmousedown="startColumnResize(event, '<?= $column_id ?>'); event.stopPropagation(); event.preventDefault()"
                     ondblclick="autoResize('<?= $column_id ?>')"
                     data-context-menu="{'Ottimizza':function(){ autoResize('<?= $column_id ?>'); }, 'Ottimizza colonne':function(){ autoResize(false); }}"></div>
	...
	foreach ($this->options['data']['elements'] as $id => $el) {
		...
		?>>
            <div class="results-table-row" data-n="<?= $c_row++ ?>" data-id="<?= $id ?>"
                ...
                <div class="special-cell" onmousedown="event.stopPropagation()" onmouseup="event.stopPropagation()"
                     onclick="event.stopPropagation(); var check = this.firstElementChild.firstElementChild; check.getValue().then((v) => {if(v) check.setValue(0); else check.setValue(1); });">
                    <div>
                        <input type="checkbox" value="1" id="row-checkbox-<?= $id ?>" data-id="<?= $id ?>"
                               onchange="selectRow('<?= $id ?>', this.checked ? 1 : 0)"
                               onclick="event.stopPropagation()"
                               onmousedown="if(event.shiftKey){ holdRowsSelection(this); } event.stopPropagation()"
                               onmouseup="event.stopPropagation()"
                               onmouseover="if(holdingRowsSelection!==null) this.setValue(holdingRowsSelection)"
                               onkeydown="moveBetweenRows(this, event.keyCode)"/>
                    </div>
                </div>
				<?php
				...
				if ($canDelete) {
                    ...
                    <div class="special-cell" onmousedown="event.stopPropagation()" onclick="event.stopPropagation()">
                        <div>
                            <a href="#" onclick="event.stopPropagation(); deleteRows(['<?= $id ?>']); return false"><img
                                        src="<?= PATH ?>model/AdminTemplateEditt/files/img/delete.png" alt=""
	...
	if (!empty($totals)) {
        ...
        <div class="results-table-row" style="top: 0px" id="totals-row">
			...
			foreach ($this->options['data']['columns'] as $column_id => $f) {
				...
				?>
            <div style="width: <?= $this->model->_ResizeTable->widths[$column_id] ?>px"
                 data-column="<?= $column_id ?>">
                <div><?php
					if (isset($totals[$column_id])) {
						if ($f['price'])
